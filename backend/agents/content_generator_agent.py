...
class TemplateContentGeneratorAgent:
    ...
    def generate_fallback_latex(self, resume_data: ResumeData, template_name: str = "Fallback") -> str:
        """
        Fallback manual LaTeX generation when AI fails.
        """
        latex_parts = [
            ...
            "\\usepackage{hyperref}",
            "\\usepackage{titlesec}",
            "\\usepackage{url}",  # Always include url package
            "\\usepackage{xcolor}",
            "",
        ...
        # Header
        if resume_data.name.firstName or resume_data.name.lastName:
            name = f"{resume_data.name.firstName} {resume_data.name.lastName}".strip()
            latex_parts.append(f"\\begin{{center}}")
            latex_parts.append(f"{{\\Large \\textbf{{{self.escape_latex(name)}}}}}\\\\[5pt]")
            ...
            latex_parts.extend(["\\end{center}", ""])
        
        # About Me section
        if resume_data.aboutMe.description or resume_data.aboutMe.polishedDescription:
            about_text = resume_data.aboutMe.polishedDescription or resume_data.aboutMe.description
            if about_text:
                latex_parts.extend([
                    "\\section*{Summary}",
                    self.escape_latex(about_text),
                    ""
                ])
        ...
        if resume_data.experience:
            ...
            for exp in resume_data.experience:
                if exp.company or exp.position:
                    position = exp.position or exp.title or ""
                    company = exp.company or ""
                    dates = f"{exp.startDate or ''} - {exp.endDate or 'Present'}" if exp.startDate else ""
                    
                    latex_parts.append(f"\\textbf{{{self.escape_latex(position)}}}")
                    if company:
                        latex_parts.append(f"\\hfill {self.escape_latex(company)}")
                    if dates:
                        latex_parts.append(f"\\\\{self.escape_latex(dates)}")
                    
                    if exp.description:
                        latex_parts.append(f"\\\\{self.escape_latex(exp.description)}")
        ...
        if resume_data.education:
            ...
            for edu in resume_data.education:
                if edu.institution or edu.degree:
                    ...
                    dates = f"{edu.startDate or ''} - {edu.endDate or edu.graduationDate or ''}" if edu.startDate or edu.graduationDate else ""
                    
                    latex_parts.append(f"\\textbf{{{self.escape_latex(degree)}}}")
                    if institution:
                        latex_parts.append(f"\\hfill {self.escape_latex(institution)}")
                    if dates:
                        latex_parts.append(f"\\\\{self.escape_latex(dates)}")
        ...
        if resume_data.skills:
            ...
            for skill in resume_data.skills:
                if skill.name or skill.skill:
                    skill_text = skill.name or skill.skill
                    skills.append(self.escape_latex(skill_text))
            
            if skills:
                latex_parts.append(", ".join(skills))
                latex_parts.append("")
        
        # Custom sections
        for custom in resume_data.customSections:
            if custom.title and custom.content:
                latex_parts.append(f"\\section*{{{self.escape_latex(custom.title)}}}")
                latex_parts.append(self.escape_latex(custom.content))
                latex_parts.append("")
